...
export default function ProductCatalog({ setSelectedProduct }: ProductCatalogProps = {}) {
  ...
  const categorizedProducts = useMemo(() => {
    ...
    products.forEach(p => {
      ...
      } else if (cats.includes("led table lamp") || cats.includes("led usb lamp") || cats.includes("corporate gifting")) {
        groups["rechargeable"].push(p);
      } else if (cats.includes("led lantern") || cats.includes("solar energy kit")) {
        groups["emergency-lights"].push(p);
      } else if (cats.includes("led headlamp") || cats.includes("solar lantern & searchlight")) {
  ...
  return (
    <section id="catalog" className="py-24 bg-white scroll-mt-20">
                  ...
                  <div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                      {visibleProducts.map((p, pIdx) => {
                        ...
                        return (
                          <motion.div
                            key={p.id}
                            initial={{ opacity: 0, filter: "blur(6px)", y: 25 }}
                            whileInView={{ opacity: 1, filter: "blur(0px)", y: 0 }}
                            viewport={{ once: true, margin: "-40px" }}
                            transition={{ duration: 0.45, delay: (pIdx % 4) * 0.08, ease: "easeOut" }}
                    ...
                    {list.length > 8 && (
                      <div className="flex justify-center mt-10">
                        <button
                          onClick={() => toggleCategoryExpand(currentWebCat.id)}
                        ...
                        >
                          {isExpanded ? "Show Less" : `View All ${list.length} ${currentWebCat.label}`}
                          <svg
